...
def pdf_to_markdown(pdf_path: str) -> dict:
    """Extracts text from a PDF and returns serializable data using PyPDFLoader. Assumes text-based; for tables, text is extracted as-is."""
    try:
...
def excel_to_markdown(excel_path: str) -> dict:
    """Extracts tables from Excel sheets and converts to Markdown."""
    try:
        ...
        if not content.strip():
            logging.info(f"Extracted content is empty for Excel: {excel_path}")
            return {"content": "", "metadata": {}}
        logging.info(f"Extracted Markdown from Excel {excel_path}: {content[:100]}...")
        return {
            "content": content,
            "metadata": {}
        }
    except Exception as e:
        logging.error(f"Error extracting Excel {excel_path}: {str(e)}", exc_info=True)
        return {"content": str(e), "metadata": {}}
            ...
            for part in msg_data["payload"].get("parts", []):
                if part.get("filename") and part.get("body", {}).get("attachmentId"):
                    ...
                    if mime_type == "application/pdf":
                        ...
                        extracted_content = excel_to_markdown(attachment_path)
                        logging.info(f"Extracted Markdown from Excel {filename}: {extracted_content['content'][:100]}...")
                    elif mime_type in ["image/png", "image/jpeg", "image/jpg"]:
                        base64_content = image_to_base64(attachment_path)
                        logging.info(f"Converted image {filename} to base64")
                    else:
                        logging.warning(f"Unsupported mime_type {mime_type} for {filename}")
                        extracted_content = {"content": "Unsupported file type", "metadata": {}}
                    attachments.append({
                        "filename": filename,
                        "mime_type": mime_type,
...
def trigger_response_tasks(**kwargs):
    """Trigger 'cloud_assess_send_response_email' for each unread email with attachments."""
    ti = kwargs['ti']
    unread_emails = ti.xcom_pull(task_ids="fetch_unread_emails", key="unread_emails")
    ...
    for email in unread_emails:
        ...
        trigger_task = TriggerDagRunOperator(
            task_id=task_id,
            trigger_dag_id="cloud_assess_send_response_email",
