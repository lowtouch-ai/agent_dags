...
def generate_voice_message_agent(loan_id, agent_url, transcription=None, inserted_date=None):
    ...
    if transcription and inserted_date:
        ...
        prompt = (
            f"Generate a professional loan due reminder message for loan ID {loan_id}. Fetch overdue details including customerid, loanamount, interestrate, tenureinmonths, outstandingamount, overdueamount, lastduedate, lastpaiddate, and daysoverdue. If details are unavailable, use placeholders (e.g., 'Customer', 'N days'). Convert the message into this template: 'Dear (Name), this is a gentle reminder of your loan number ({loan_id}) which is overdue by (n) installments. Kindly tell us when you can make the payments, after the beep.' Keep the final message under 500 characters."
        )
