...
def fetch_due_loans(api_url, test_phone_number, **kwargs):
    ...
    ti = kwargs['ti']
    try:
        logger.info(f"Calling API to fetch due loans: {api_url}loan/get_reminder?status=Reminder")
        response = requests.get(f"{api_url}loan/get_reminder?status=Reminder")
        if response.status_code == 200:
            ...
            for reminder in loans_reminders:
                ...
                customer_response = requests.get(f"{api_url}customer/{customer_id}")
                if customer_response.status_code == 200:
                    # Use test_phone_number as a placeholder
    ...
    for loan in loans:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Failed to update reminder_status to CallInitiated for loan ID: {loan_id}. "
        ...
        # Generate the voice message by calling the AgentOmatic API
...
def update_call_status(api_url, **kwargs):
    ...
    for call_id in call_ids:
        ...
        try:
            ...
            logger.error(f"Failed to update status. call_id={call_id}, loan_id={loan_id} due to exception: {str(e)}")

        # OPTIONAL: Delete the Variable to prevent clutter
