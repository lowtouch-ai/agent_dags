...
with DAG(
    ...
    def fetch_recording_task_func(**kwargs):
        ...
        if call_id:
            if "transcription" in result and result["transcription"] not in ["No transcription available", "Transcription failed", "Transcription pending"]:
            ...
            if "file_path" in result:
                Variable.set(f"twilio_recording_file_{call_id}", result["file_path"])
                logger.info(f"Set Variable twilio_recording_file_{call_id} to: {result['file_path']}")
        ...
        if call_id and "file_path" in result:
            ti.xcom_push(key=f"recording_file_{call_id}", value=result["file_path"])  # Fixed syntax
        return result
