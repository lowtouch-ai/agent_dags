...
with DAG(
    ...
    def initiate_call(phone_number, message, need_ack, call_id, **kwargs):
        """Initiate the voice call using Twilio API with speech recognition."""
        ti = kwargs["ti"]
        ...
        try:
            if need_ack:
                ...
                <Response>
                    <Say>{message}</Say>
                    <Gather input="speech" timeout="5" speechTimeout="auto" actionOnEmptyResult="true">
    ...
    def check_call_status(**kwargs):
        """Poll Twilio for call status and transcription."""
        ti = kwargs["ti"]
        conf = kwargs["params"]
        ...
        if current_status in ["completed", "no-answer", "busy", "failed"]:
            ...
            if need_ack and current_status == "completed":
                transcriptions = client.transcriptions.list(call_sid=call_sid)
                if transcriptions:
                    transcription_text = transcriptions[0].transcription_text
                    logger.info(f"Transcription for call SID={call_sid}: {transcription_text}")
                    ti.xcom_push(key="transcription_text", value=transcription_text)
                else:
                    logger.info(f"No transcription available yet for call SID={call_sid}")
                    ti.xcom_push(key="transcription_text", value="No transcription available")
    ...
    def fetch_and_save_recording(base_storage_dir, **kwargs):
        """Fetch and save recording from Twilio, log and include transcription."""
        ti = kwargs["ti"]
        conf = kwargs["params"]
        ...
        need_ack = ti.xcom_pull(task_ids="initiate_call", key="need_ack")
        call_id = conf.get("call_id", None)
        transcription_text = ti.xcom_pull(task_ids="check_call_status", key="transcription_text")
        ...
        if not recordings:
            ...
            return {"message": "Recording not available yet. Try again later.", "transcription": transcription_text}
        ...
        if transcription_text:
            logger.info(f"Transcribed text from recording for call SID={call_sid}: {transcription_text}")
        else:
            logger.info(f"No transcribed text available for call SID={call_sid}")
        ...
        try:
            ...
            return {
                ...
                "file_path": file_path,
                "transcription": transcription_text if transcription_text else "No transcription available"
            }
        except Exception as e:
