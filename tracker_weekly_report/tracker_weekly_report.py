...
SMTP_HOST = Variable.get("ltai.v1.tracker.SMTP_HOST", default="mail.authsmtp.com")
SMTP_PORT = int(Variable.get("ltai.v1.tracker.SMTP_PORT", default="2525"))
SMTP_SUFFIX = Variable.get("ltai.v1.tracker.SMTP_FROM_SUFFIX", default="via lowtouch.ai <webmaster@ecloudcontrol.com>")
SENDER_EMAIL = Variable.get("ltai.v1.tracker.FROM_ADDRESS", default=SMTP_USER)
...
hr_email = Variable.get("ltai.v1.tracker.whitelisted.hr_email", default=None)
manager_email = Variable.get("ltai.v1.tracker.whitelisted.manager_email", default=None)
admin_email = Variable.get("ltai.v1.tracker.whitelisted.admin_email", default=None)
...
def calculate_week_range(ti, **context):
    ...
    # Default: calculate previous week from logical_date
...
def filter_users_by_timezone(ti, **context):
    ...
    Window: 8 AM - 11:59 PM IST on Monday (enforced)
    ...
    ist_time = logical_date.in_timezone(ist_tz)

    logging.info(f"=" * 80)
    logging.info(f"FILTERING USERS (IST Window: 8 AM - 11:59 PM Monday)")
    logging.info(f"=" * 80)
    logging.info(f"Current UTC: {logical_date.format('YYYY-MM-DD HH:mm:ss')}")
    ...
    # CRITICAL SAFEGUARD 2: Ensure we're within IST window (8 AM - 11:59 PM)
    if not (8 <= ist_time.hour < 24):
        logging.warning(f"Outside IST window (8 AM - 11:59 PM). Current hour: {ist_time.hour}:00. Skipping all users.")
        ti.xcom_push(key="users_to_process", value=[])
        ti.xcom_push(key="total_users_to_process", value=0)
        return []
    ...
    for user in users:
        ...
        try:
            ...
            user_local_time = logical_date.in_timezone(user_tz)
...
with DAG(
    ...
    schedule='30 2-6 * * 1',  # Mon 2:30-6:30 AM UTC = 8:00 AM-12:00 PM IST
    catchup=False,
