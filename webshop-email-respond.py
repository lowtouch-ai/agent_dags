...
def get_email_thread(service, email_data):
    try:
        ...
        if not thread_id:
            ...
            return [{"headers": email_data["headers"], "content": clean_html_content(email_data["content"])}]

        thread = service.users().threads().get(userId="me", id=thread_id).execute()
        email_thread = [{
                "headers": {header["name"]: header["value"] for header in msg.get("payload", {}).get("headers", [])},
                "content": clean_html_content(decode_email_payload(msg.get("payload", {}))).strip(),
                "timestamp": msg.get("internalDate", "0")
            }
            for msg in thread.get("messages", [])]
        # Sort by timestamp to maintain chronological order
        email_thread.sort(key=lambda x: int(x["timestamp"]))
        return email_thread
    except Exception as e:
        logging.error(f"Error retrieving email thread: {str(e)}")
        return []

def get_ai_response(service, email_data):
    try:
        # Fetch email thread for context
        email_thread = get_email_thread(service, email_data)
        if not email_thread:
            logging.warning("No email thread found, using only current email content.")
            user_query = clean_html_content(email_data.get("content", "")).strip()
        else:
            # Combine thread history into a single query
            thread_context = "\n\n--- Previous Messages ---\n"
            for msg in email_thread[:-1]:  # Exclude the latest message
                sender = msg["headers"].get("From", "Unknown")
                content = msg["content"] or "No content"
                thread_context += f"From: {sender}\nContent: {content}\n\n"
            current_query = clean_html_content(email_data.get("content", "")).strip()
            user_query = f"{thread_context}--- Current Message ---\nFrom: {email_data['headers'].get('From', 'Unknown')}\nContent: {current_query}"

        if not user_query:
            logging.warning("No valid query content found.")
            return "We are currently experiencing technical difficulties. Please provide more details or try again later."

        logging.info(f"Sending query to AI: {user_query}")
...
def send_response(**kwargs):
    try:
        ...
        if not is_valid_email(sender_email):
            logging.error(f"Invalid sender email: {sender_email}")
            return
        
        subject = f"Re: {email_data['headers'].get('Subject', 'No Subject')}"
        ai_response_html = get_ai_response(service, email_data)
